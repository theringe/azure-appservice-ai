...
# Create and set up virtual environments for tensorflow-webjob and tensorflow
for env in tensorflow-webjob tensorflow; do
    VENV_PATH="$VENV_ROOT/$env"
    if [ ! -d "$VENV_PATH" ]; then
        python -m venv "$VENV_PATH"
        source "$VENV_PATH/bin/activate"
        requirements="$ROOT/tensorflow/requirements.txt"
        [ "$env" == "tensorflow-webjob" ] && requirements="$ROOT/tensorflow/webjob/requirements.txt"
