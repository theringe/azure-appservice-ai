...
for env in openai-webjob openai; do
    ...
    [ "$env" == "openai-webjob" ] && requirements="$ROOT/webjob/requirements.txt"
    $VENV_PATH/bin/pip install -r $requirements
    deactivate
done

# Start the application using the new openai virtual environment
source $NEW_VENV_FOLDER/openai/bin/activate
$NEW_VENV_FOLDER/openai/bin/python $ROOT/api/app.py
